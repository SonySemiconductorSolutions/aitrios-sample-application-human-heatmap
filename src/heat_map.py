...
class HeatMap(object_detection_processor.ObjectDetectionProcessor):
    ...
    def __init__(self, config):
        ...
        self._griddata = [[0
                           for _ in range(self._param["grid_num_h"])]
                          for _ in range(self._param["grid_num_v"])]

        if (self._param["image_size_h"] % self._param["grid_num_h"]) != 0:
            raise RuntimeError(
                "error, image_size_h is not divisible by grid_num_h")
        if (self._param["image_size_v"] % self._param["grid_num_v"]) != 0:
            raise RuntimeError(
                "error, image_size_v is not divisible by grid_num_v")
        self._param["grid_size_h"] = (self._param["image_size_h"] //
                                      self._param["grid_num_h"])
        self._param["grid_size_v"] = (self._param["image_size_v"] //
                                      self._param["grid_num_v"])
    ...
    def __call__(self, serialize_meta):
        ...
        # Output to dict
        result_dict = self._output_to_dict(bbox_array, self._griddata)
    ...
    def _dec_around_grid(self, pos_h, pos_v):
        ...
        if self._number_of_add_grid > 0:
            for h_g in range(
                    pos_h - self._number_of_add_grid,
                    pos_h + self._number_of_add_grid,
            ):
                for v_g in range(
                        pos_v - self._number_of_add_grid,
                        pos_v + self._number_of_add_grid,
                ):
                    if (0 <= h_g < self._param["grid_num_h"]) and (
                            0 <= v_g < self._param["grid_num_v"]):
    ...
    def _dec_bbox(self, data):
        ...
        for bbox in data:
            ...
            grid_r = min(grid_r + self._number_of_add_grid,
                         self._param["grid_num_h"] - 1)
            grid_b = min(grid_b + self._number_of_add_grid,
                         self._param["grid_num_v"] - 1)
