...
class HeatMapOutput(output.Output):
    ...
    def __init__(self, config, image_info, param_info):
        ...
        if isinstance(output_name, str) and output_name:
            video_file_name = os.path.join(video_output_dir, f"{output_name}_heatmap.mp4")
        else:
            video_file_name = os.path.join(video_output_dir, "heatmap.mp4")
    ...
    def __call__(self, dict_meta, image=None, timestamp=None):
        ...
        heat_data = [[0
                      for _ in range(self._param_info["image_size_h"])]
            ...
            for grid_h, _ in enumerate(dict_meta["griddata"][grid_v]):
                for v_g in range(0, self._param_info["grid_size_v"]):
                    for h_g in range(0, self._param_info["grid_size_h"]):
                        heat_data[
                            grid_v * self._param_info["grid_size_v"] +
                            v_g][grid_h * self._param_info["grid_size_h"] +
                                 h_g] = dict_meta["griddata"][grid_v][grid_h]
        ...
        plt.axis("off")
        plt.savefig("./output_tmp.jpg", bbox_inches="tight",
                    pad_inches=0)  # Axis Untitled
        plt.close()
